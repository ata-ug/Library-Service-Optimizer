...
CREATE TABLE books (
    ...
    FOREIGN KEY (shelf_location_id) REFERENCES locations(location_id)
);

-- ------------------------------------------------------------
-- 5. service_requests
-- Jobs queued/prioritised/searched/sorted by Algorithms Engine.
-- destination_location_id derived from the requested book's
-- shelf_location_id. (min 300 rows required)
-- ------------------------------------------------------------
...
-- ------------------------------------------------------------
-- 8. algorithm_runs
-- Empirical runtime/memory measurements for M10. (min 30 rows)
-- ------------------------------------------------------------
...
CREATE TABLE algorithm_parameters (
    ...
    FOREIGN KEY (member_index_number) REFERENCES members(index_number)
);

-- ============================================================
-- Indexes on foreign keys / lookup columns.
-- Not part of the rubric's core requirement, but they make the
-- M10 performance-comparison queries and joins realistic without
-- touching any of the hand-built data-structure logic.
-- ============================================================
CREATE INDEX idx_roads_from            ON roads(from_location_id);
CREATE INDEX idx_roads_to              ON roads(to_location_id);
CREATE INDEX idx_books_shelf           ON books(shelf_location_id);
CREATE INDEX idx_books_category        ON books(category);
CREATE INDEX idx_requests_member       ON service_requests(member_id);
CREATE INDEX idx_requests_book         ON service_requests(book_id);
CREATE INDEX idx_requests_status       ON service_requests(status);
CREATE INDEX idx_requests_source       ON service_requests(source_location_id);
CREATE INDEX idx_requests_destination  ON service_requests(destination_location_id);
CREATE INDEX idx_issuelogs_request     ON issue_logs(request_id);
CREATE INDEX idx_issuelogs_book        ON issue_logs(book_id);
CREATE INDEX idx_issuelogs_member      ON issue_logs(member_id);
CREATE INDEX idx_resources_home        ON resources(home_location_id);
CREATE INDEX idx_auditevents_entity    ON audit_events(entity_type, entity_id);
CREATE INDEX idx_algoparams_member     ON algorithm_parameters(member_index_number);
